...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        ...
        if (wtx.HasZerocoinPublicSpendInputs()) {
            ...
            if (!ZHXGModule::ParseZerocoinPublicSpend(wtx.vin[0], wtx, state, publicSpend)){
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zhxgTracker->HasMintTx(hash))) {
            //zHXG stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZHXG;
...
bool IsZHXGType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZHXG:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZHXG || type == TransactionRecord::MNReward) {

...
#include "ui_zhxgcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zhxgTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zhxgTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zHXG.";
            else if (!mint.isSeedCorrect)
                strReason = "The zHXG seed used to mint this zHXG is not the same as currently hold in the wallet";
            else

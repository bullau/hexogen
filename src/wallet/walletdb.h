...
public:
    ...
    bool WriteZHXGSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZHXGSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZHXGSeed_deprecated(uint256& seed);
    bool EraseZHXGSeed();
    bool EraseZHXGSeed_deprecated();

    bool WriteZHXGCount(const uint32_t& nCount);
    bool ReadZHXGCount(uint32_t& nCount);

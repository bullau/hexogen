...
CzHXGWallet::CzHXGWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zhxg seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZHXGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZHXGSeed_deprecated()) {
                    LogPrintf("%s: Updated zHXG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zhxg seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zhxg wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzHXGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZHXGCount(nCountLastUsed);
    else if (!walletdb.ReadZHXGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzHXGWallet::Lock()
{
    seedMaster = 0;
}

void CzHXGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzHXGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZHXG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzHXGWallet::LoadMintPoolFromDB()
{
...
void CzHXGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzHXGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzHXGWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zhxgTracker->HasPubcoinHash(pMint.first)) {
...
bool CzHXGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZHXG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zhxgTracker which also adds to database
    pwalletMain->zhxgTracker->Add(dMint, true);
...
void CzHXGWallet::SeedToZHXG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzHXGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzHXGWallet::GenerateDeterministicZHXG(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzHXGWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzHXGWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzHXGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());

...
CzHXGTracker::CzHXGTracker(std::string strWalletFile)
{
...
CzHXGTracker::~CzHXGTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzHXGTracker::Init()
{
...
bool CzHXGTracker::Archive(CMintMeta& meta)
{
...
bool CzHXGTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzHXGTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzHXGTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzHXGTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzHXGTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzHXGTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzHXGTracker::GetSerialHashes()
{
...
CAmount CzHXGTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzHXGTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzHXGTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzHXGTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzHXGTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzHXGTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzHXGTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzHXGTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzHXGTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzHXGTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzHXGTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzHXGWallet* zHXGWallet)
{
    bool iszHXGWalletInitialized = (NULL != zHXGWallet);
    CMintMeta meta;
    ...
    if (! iszHXGWalletInitialized)
        zHXGWallet = new CzHXGWallet(strWalletFile);
    meta.isSeedCorrect = zHXGWallet->CheckSeed(dMint);
    if (! iszHXGWalletInitialized)
        delete zHXGWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzHXGTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzHXGTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzHXGTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzHXGTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzHXGTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzHXGTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzHXGWallet* zHXGWallet = new CzHXGWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zHXGWallet);
        }
        delete zHXGWallet;
        LogPrint("zero", "%s: added %d dzhxg from DB\n", __func__, listDeterministicDB.size());
    }

...
// The zHXG block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zHXG is a hash of the serial
...
{
    //Create an output returning the zHXG that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZHXGOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zHXG output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zHXG", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZHXGOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zHXG output", __func__);
...
{
    CzHXGTracker* zhxgTracker = pwallet->zhxgTracker.get();
    CMintMeta meta;
    if (!zhxgTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zhxgTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a HXG stake is the outpoint
